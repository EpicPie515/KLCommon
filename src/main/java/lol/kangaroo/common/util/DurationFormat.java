...
public class DurationFormat {
	...
	public static String getFormatted1UnitDuration(Duration dur, Locale lang, boolean past) {
		long days = dur.toDays();
		long years = ((days - (days % 365)) / 365);
		dur = dur.minusDays(days);
		days = days % 365;
		...
		if(years == 0 && days == 0 && hours == 0 && minutes == 0)
			...
			if(years > 0) ds = years + " " + MSG.TIMEFORMAT_YEARS.getMessage(lang);
			else if(days > 0) ds = days + " " + MSG.TIMEFORMAT_DAYS.getMessage(lang);
			else if(hours > 0) ds = hours + " " + MSG.TIMEFORMAT_HOURS.getMessage(lang);
			else ds = minutes + " " + MSG.TIMEFORMAT_MINUTES.getMessage(lang);
		}
		if(past) ds += " " + MSG.TIMEFORMAT_PAST.getMessage(lang);
		return ds;
	}
